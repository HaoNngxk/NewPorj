...
import com.xkyb.jf.http.RetrofitHttp;
...
/*    public static void getProducts(Callback<ResponseBody> responseBodyCallback) {
        OkHttpClient.Builder builder = new OkHttpClient.Builder();
        RetrofitHttp.getRetrofit(builder.build()).getType("classifyinfo").enqueue(responseBodyCallback);
    }*/

    public static void getProducts(Subscriber<MsgData<List<HomeType>>> subscriber) {
        OkHttpClient.Builder builder = new OkHttpClient.Builder();
        RetrofitHttp.getRetrofit(builder.build()).getType("classifyinfo")
                .subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(subscriber);
